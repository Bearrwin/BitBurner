...
export async function main(ns) {
    ...
    if (ns.getHackingLevel() > ns.getServerRequiredHackingLevel("run4theh111z") && ns.hasRootAccess("run4theh111z") && run4Backdoored == false) {
     ...
     let fulcrumBackdoored = ns.getServer("fulcrumassets").backdoorInstalled
    if (ns.getHackingLevel() > ns.getServerRequiredHackingLevel("fulcrumassets") && ns.hasRootAccess("fulcrumassets") && fulcrumBackdoored == false) {
        await ns.exec("/utils/backdoor.js", "home", 1, "fulcrumassets")
        await ns.sleep(15000)
    }
}