...
export async function main(ns) {
  ...
  function serverList(ns, current = "home", set = new Set()) {
	...
	// 	run /init/init.batcher.hnetPool.js phantasy 10 25 10 1000 
  ...
  //ns.ui.moveTail(1450, 540)
	...
	let hnetServList = serverList(ns).filter(s => s.startsWith("hacknet"))
	...
	if (secHigh > 2) {
		while (breakAmmoCounter > 0) {
			...
			for (let server of hnetServList) {
	...
	while (true) {
		...
		hnetServList = serverList(ns).filter(s => s.startsWith("hacknet"))
