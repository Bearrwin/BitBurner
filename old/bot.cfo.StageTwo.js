...
export async function main(ns) {
	...
	const initialServPurchCount = 25
	...
	const secondServPurchSize = ninthServUpgServerSize
	...
	const homeRamUpgStageTwoMoney = 4700000000000
	let homeCoresUpg = false
	const homeCoresUpgSize = 4
	const homeCoresUpgMoney = 490000000000
	let augPurchStageTwo = false
	const augPurchStageTwoMoney = 1550000000000
	...
	if (ns.getServerMaxRam(purchServerName) < ninthServUpgServerSize) {
		...
		if (ninthServUpg == false) {
			ns.tprint("Saving up until I have " + `${ns.formatNumber(ninthServUpgMoney)} `);
			ns.tprint("");
			while (ns.getServerMoneyAvailable("home") < ninthServUpgMoney && ninthServUpg == false) {
				await ns.sleep(5000)
			}
			ns.run("/serv/serv.purchasedupg.auto.js", 1, ninthServUpgServerSize)
			await ns.sleep(5000)
			ns.tprint("Upgrading RAM on our purchased servers it is now.. " + ns.getServerMaxRam(purchServerName));
			await ns.sleep(5000)
		}
...
// 	homeCoresUpg = true
	...
	// 		await ns.sleep(2000)
	// 		ns.run("/utils/faction.buy.augment.js", 1, "Tian Di Hui", "Neuroreceptor Management Implant")
	// 		await ns.sleep(2000)
